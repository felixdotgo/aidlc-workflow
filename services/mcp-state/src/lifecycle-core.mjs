...
const parseJsonFile = (path, label) => { try { return JSON.parse(readFileSync(path, "utf8")); } catch (error) { throw new Error(`${label} is corrupted or unreadable: ${error instanceof Error ? error.message : String(error)}; restore it from git history or the archive records`); } };
...
export const loadState = (root) => { assertNoSymlinkPath(root, ".agents/data/state/aidlc-state.json"); return existsSync(statePath(root)) ? validateState(parseJsonFile(statePath(root), "AI-DLC state file")) : emptyState(); };
