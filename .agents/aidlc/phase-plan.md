...
6. Set task status to `blocked_on_user`, run `node .agents/aidlc/scripts/gate-check.mjs <task-id> --gate G1_review`, present the 🔵 G1 form, and stop unless the strict low-risk auto-pass policy applies.
7. Apply the user's resolutions through `node .agents/aidlc/scripts/state.mjs decision set`. Record `approval/G1_review/pass`; transition to `build` only when no decision remains unresolved.
