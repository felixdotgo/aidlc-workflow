...
const latest = (task, predicate, after) => task.evidence.filter((item) => predicate(item) && (!after || Date.parse(item.recordedAt) > Date.parse(after))).reduce((current, item) => !current || Date.parse(item.recordedAt) >= Date.parse(current.recordedAt) ? item : current, undefined);
export const latestApproval = (task, gate) => latest(task, (item) => item.kind === "approval" && item.gate === gate);
export const buildBoundary = (task) => latestApproval(task, "G1_review")?.recordedAt;
export const hasApproval = (task, gate) => latestApproval(task, gate)?.result === "pass";
export const hasAreaVerification = (task, area) => latest(task, (item) => ["test", "lint"].includes(item.kind) && (item.area === area || (!item.area && task.areas.length === 1)), buildBoundary(task))?.result === "pass";
export const hasVerification = (task) => task.areas.every((area) => hasAreaVerification(task, area));
export const hasReview = (task) => latest(task, (item) => item.kind === "review", buildBoundary(task))?.result === "pass";
...
export const transitionDiagnostics = (task, target) => {
  ...
  if (target === "wrap") { if (!hasVerification(task)) diagnostics.push({ level: "ERROR", code: "VERIFY_EVIDENCE", message: "Latest post-G1 test or lint evidence must pass for every affected area" }); if (!hasReview(task)) diagnostics.push({ level: "ERROR", code: "REVIEW_EVIDENCE", message: "Latest post-G1 code-review evidence must pass" }); if (!hasApproval(task, "G2_codereview")) diagnostics.push({ level: "ERROR", code: "G2_APPROVAL", message: "G2 approval evidence is required" }); }
