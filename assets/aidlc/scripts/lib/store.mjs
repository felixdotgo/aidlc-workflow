...
export const isGrandfatheredG2Wait = (task) => task.phase === "build" && task.gate === "G2_codereview" && task.status === "blocked_on_user" && task.tasks.every((item) => ["done", "deferred"].includes(item.status)) && hasVerification(task) && hasReview(task);
