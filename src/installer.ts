...
export const detectedAgents = (root: string): AgentId[] => adapters.filter((adapter) => adapter.id !== "generic" && adapter.detect(root)).map((adapter) => adapter.id);
...
export const doctor = (root: string): string => {
  const manifest = join(resolve(root), ".agents/aidlc/manifest.json");
  if (!existsSync(manifest)) return "ERROR: .agents/aidlc/manifest.json is missing. Run aidlc init --agent <agent> --yes.";
