...
test("publish evidence accepts one pinned passing runner", () => {
  ...
  try {
    ...
    writeFileSync(path, JSON.stringify({ packageVersion: "1.0.1", createdAt: new Date().toISOString(), reports: [report("a"), report("b")] }));
    assert.equal(verifyReleaseEvidence(path, "1.0.1").reports.length, 2);
    writeFileSync(path, JSON.stringify({ packageVersion: "1.0.1", createdAt: new Date().toISOString(), reports: [report("a")] }));
    assert.equal(verifyReleaseEvidence(path, "1.0.1").reports.length, 1);
