...
test("publish evidence accepts one pinned passing runner", () => {
  ...
  try {
    ...
    writeFileSync(path, JSON.stringify({ packageVersion: "2.2.0", createdAt: new Date().toISOString(), reports: [report("a"), report("b")] }));
    assert.equal(verifyReleaseEvidence(path, "2.2.0").reports.length, 2);
    writeFileSync(path, JSON.stringify({ packageVersion: "2.2.0", createdAt: new Date().toISOString(), reports: [report("a")] }));
    assert.equal(verifyReleaseEvidence(path, "2.2.0").reports.length, 1);
  } finally { rmSync(root, { recursive: true, force: true }); }
});
